...
model = tree.DecisionTreeClassifier(max_depth=3, class_weight='balanced', criterion='entropy', min_samples_leaf=10)
...
#%%
model = tree.DecisionTreeClassifier(max_depth=3, class_weight='balanced', criterion='entropy', min_samples_leaf=10)
path = model.cost_complexity_pruning_path(df.drop('target', axis=1).values, y_train.values)
ccp_alphas, impurities = path.ccp_alphas, path.impurities

clfs = []
for ccp_alpha in ccp_alphas:
    clf = tree.DecisionTreeClassifier(max_depth=3, class_weight='balanced', criterion='entropy', min_samples_leaf=10, ccp_alpha=ccp_alpha)
...
train_scores = [clf.score(df.drop('target', axis=1).values, y_train.values) for clf in clfs]
test_scores = [clf.score(test_df.drop('target', axis=1).values, y_test) for clf in clfs]
...
model = ExtraTreesClassifier(criterion='entropy', max_depth=3, min_samples_leaf=10, class_weight='balanced')
model = model.fit(df.drop('target', axis=1).values, y_train.values)
y_test = test_df['target']
y_pred = model.predict(test_df.drop('target', axis=1).values)
...
clf = GridSearchCV(ExtraTreesClassifier(), params, scoring='f1', cv=5, refit=True, return_train_score=True)
clf.fit(df.drop('target', axis=1).values, y_train)
print('Best-params:', clf.best_params_)
print('Best-score:', clf.best_score_)
print("Accuracy: " + str(accuracy_score(y_test, clf.predict(test_df.drop('target', axis=1).values))))
...
clf = GridSearchCV(pipe, params, scoring='f1', cv=5, refit=True, return_train_score=True)
clf.fit(df.drop('target', axis=1).values, y_train)
print('Best-params:', clf.best_params_)
print('Best-score:', clf.best_score_)
print("Accuracy: " + str(accuracy_score(y_test, clf.predict(test_df.drop('target', axis=1).values))))
